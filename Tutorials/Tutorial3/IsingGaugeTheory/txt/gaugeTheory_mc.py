...
### Tutorial 3: Monte Carlo for the Ising lattice gauge theory
#####################################################################################
...
### Input parameters: ###
T_list = [0, np.inf]#np.linspace(5.0,0.5,19) #temperature list
...
def getPlaquetteProduct(i):
  ...
  # ************************************************************** #
  return spins[2*i]*spins[2*i+1]*spins[2*neighbours[i,0]]*spins[2*neighbours[i,0]+1]
...
def sweep():
  ...
  for i in range(N_sites):
    ...
    starLoc = random.randint(0,N_sites-1)
    spins[2*starLoc] = -spins[2*starLoc]
    spins[2*starLoc+1] = -spins[2*starLoc+1]
    spins[2*neighbours[starLoc,0]] = -spins[2*neighbours[starLoc,0]]
    spins[2*neighbours[starLoc,0]+1] = -spins[2*neighbours[starLoc,0]+1]
...
for T in T_list:
  ...
  for i in range(n_bins):
    ...
    if (i+1)%50==0:
      print('{} bins complete'.format(i+1))
  #end loop over i
